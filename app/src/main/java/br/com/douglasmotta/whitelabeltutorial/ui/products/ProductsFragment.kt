...
class ProductsFragment : Fragment() {
    ...
    private fun observeNavBackStack() {
        findNavController().run {
            ...
            val savedStateHundle = navBackStackEntry.savedStateHandle
            val observer = LifecycleEventObserver { _, event ->
                if (event == Lifecycle.Event.ON_RESUME && savedStateHundle.contains(PRODUCT_KEY)) {
                    val product = savedStateHundle.get<Product>(PRODUCT_KEY)
                    ...
                    savedStateHundle.remove<Product>(PRODUCT_KEY)
                }
            }
